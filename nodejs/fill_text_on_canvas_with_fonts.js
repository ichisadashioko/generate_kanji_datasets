...
function normalize_filename(filename) {
    ...
    var ext = filename.substring(ext_idx)
    var filename = filename.substring(0, ext_idx)
...
function get_font_list(path = 'kanji_fonts') {

    let retval = []

    var file_list = fs.readdirSync(path)
    for (let i = 0; i < file_list.length; i++) {
        let file_path = `${path}/${file_list[i]}`

        let font_dict = new Map()
        font_dict.path = file_path
        font_dict.family = make_id(4)

        registerFont(font_dict.path, { family: font_dict.family })
        console.log(font_dict)

        retval.push(font_dict)
    }
 ...
 */
function save_canvas(canvas, file_path, overwrite = false) {
    if (fs.existsSync(file_path) && !overwrite) {
        console.log(`${file_path} is already existed.`)
        return
    }
...
/**
 * 
 * @param {Array} kanji_arr 
 * @param {Array} font_list 
 * @param {String} image_path 
 */
function canvas_pipeline(kanji_arr, font_list, image_path = 'kanji_images') {

    const WIDTH = 320, HEIGHT = 320
    var text_style = 'rgb(255,255,255)'
    var background_style = 'rgb(0,0,0)'

    /**
     * Step 0: Select the kanji
     */
    for (let i = 0; i < kanji_arr.length; i++) {
        let kanji = kanji_arr[i]

        let category_path = `${image_path}/${kanji}`
         ...
         */
        for (let j = 0; j < font_list.length; j++) {
            let font = font_list[j]

            /**
             * Step 2: Select font size
             */
            for (let font_size = 100; font_size <= 300; font_size += 10) {

                /**
                 * Step 3: Select rotate degree
                 */
                for (let rotate_degree = -10; rotate_degree <= 10; rotate_degree++) {

                    /**
                     * Step 4: Draw text on canvas
                     */
                    ...
                    ctx.rotate(degree_to_radian(rotate_degree))
                    ctx.translate(-WIDTH / 2, -HEIGHT / 2)
                    ...
                    let textInfo = ctx.measureText(kanji)

                    let actual_width = Math.abs(textInfo.actualBoundingBoxRight - textInfo.actualBoundingBoxLeft)
                    let actual_height = Math.abs(textInfo.actualBoundingBoxDescent - textInfo.actualBoundingBoxAscent)

                    let offset_x = (WIDTH - actual_width) / 2
                    let offset_y = (HEIGHT + actual_height) / 2

                    ctx.fillText(kanji, offset_x, offset_y)

                    /**
                     * Draw bounding box (optional)
                     */
                    // ctx.strokeStyle = 'rgb(0,255,0)'
                    // ctx.strokeRect(offset_x, offset_y - actual_height, actual_width, actual_height)

                    /**
                     * Save canvas to image
                     */

                    let filename = `${kanji}_${font.path}_${font_size}_${rotate_degree}.png`
                    filename = normalize_filename(filename)

                    let file_path = `${category_path}/${filename}`
                    file_path = normalize_path(file_path)

                    save_canvas(canvas, file_path)
                }
            }
        }
    }
 ...
 * @param {Array} font_list 
 */
function create_canvas_image(kanji, font_list) {

    const WIDTH = 320, HEIGHT = 320
    var text_style = 'rgb(255,255,255)'
    var background_style = 'rgb(0,0,0)'
    var rotate_degree = -10
    var font_size = 100
    ...
    font_list.forEach(font => {
        ...
        save_canvas(canvas, filename, true)
    })
}

var font_list = get_font_list('kanji_fonts')
// create_canvas_image('愛', font_list)
...
canvas_pipeline(KANJI_LIST, font_list)